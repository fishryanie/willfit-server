...
const resolveNotificationAudience = async (accountTokenModel: Model<IAccountToken>, payload: { accountIds?: unknown[]; sendToAll?: boolean }) => {
...
const createNotificationInboxEntries = async (notificationInboxModel: Model<INotificationInbox>, notificationId: string, accountIds: string[]) => {
...
export class NotificationService {
  ...
  async createNotificationWithAudience(payload: Record<string, unknown>) {
    ...
    const accountIds = await resolveNotificationAudience(this.accountTokenModel, { accountIds: privateWith, sendToAll })
    const notification = await this.notifyModel.create({
    ...
    const inboxCount = await createNotificationInboxEntries(this.notificationInboxModel, notification._id.toString(), accountIds)

    if (sendPush && accountIds.length > 0) {
      this.pushQueueService.enqueueSystemPush({
        accountIds,
        notification: {
